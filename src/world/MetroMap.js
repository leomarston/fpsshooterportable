...
export class MetroMap {
  ...
  _lights(b, center, mainY) {
    ...
    const z0 = b.min.z + 4, z1 = b.max.z - 4;
    const y = Math.min(b.max.y - 0.6, mainY + 2.9);  // just above the platform
    const n = Math.max(4, Math.round((z1 - z0) / 9));
    for (let i = 0; i <= n; i++) {
      const z = z0 + (z1 - z0) * (i / n);
      const L = new THREE.PointLight(0xffe9c4, 60, 34, 2.0);
      L.position.set(cx, y, z);
      this.group.add(L);
    }
    // a couple of broad fills so the whole platform reads
    for (const t of [0.3, 0.7]) {
      const f = new THREE.PointLight(0xbcd0ff, 22, 90, 1.4);
      f.position.set(cx, mainY + 1.8, b.min.z + (b.max.z - b.min.z) * t);
      this.group.add(f);
    }
  }

  // Walkable floor sample points (the platform/concourse level).
  _floorPoints(world, b) {
    const pts = [];
    for (let x = b.min.x + 1; x <= b.max.x - 1; x += 1.2) {
      for (let z = b.min.z + 1; z <= b.max.z - 1; z += 1.2) {
  ...
  _layout(allFloor, b) {
    ...
    const zc = (b.min.z + b.max.z) / 2, zr = (b.max.z - b.min.z);
    const near = (z) => floor.reduce((best, p) => (Math.abs(p.x - medX) + Math.abs(p.z - z) < Math.abs(best.x - medX) + Math.abs(best.z - z) ? p : best), floor[0]);
    const sites = {
      A: { center: near(zc - zr * 0.16).clone(), radius: 8 },
      B: { center: near(zc + zr * 0.16).clone(), radius: 8 },
    };
